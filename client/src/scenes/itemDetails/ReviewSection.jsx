...
const ReviewSection = ({ itemId }) => {
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      // First, create the review without photos
      const reviewResponse = await fetch("http://localhost:1337/api/reviews", {
        method: "POST",
        headers: {
      ...
      if (newReview.photos.length > 0) {
        ...
        formData.append("refId", reviewData.data.id);
  ...
  const filteredReviews =
    filterRating === "all"
      ? reviews
      : reviews.filter(
          (review) => review.attributes.rating === parseInt(filterRating)
        );
                  ...
                  <Box mt={2} display="flex" gap={2} flexWrap="wrap">
                    {review.attributes.photos.data.map((photo, index) => (
                      <img
                        ...
                        onClick={() =>
                          window.open(
                            `http://localhost:1337${photo.attributes.url}`,
                            "_blank"
                          )
                        }
                      />
